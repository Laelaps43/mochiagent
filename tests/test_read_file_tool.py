...
async def test_read_file_reads_requested_chunk(tmp_path: Path) -> None:
    ...
    result = _result_map(await ReadFileTool().execute(path=str(file_path), offset=2, limit=3))
...
async def test_read_file_uses_max_chars_when_limit_missing(tmp_path: Path) -> None:
    ...
    result = _result_map(await ReadFileTool().execute(path=str(file_path), offset=-5, max_chars=2))
...
async def test_read_file_clamps_offset_and_limit(tmp_path: Path) -> None:
    ...
    result = _result_map(await ReadFileTool().execute(path=str(file_path), offset=99, limit=0))
...
async def test_read_file_rejects_missing_path(tmp_path: Path) -> None:
    ...
    result = _result_map(await ReadFileTool().execute(path=str(file_path)))
...
async def test_read_file_rejects_directory_path(tmp_path: Path) -> None:
    result = _result_map(await ReadFileTool().execute(path=str(tmp_path)))
...
async def test_read_file_rejects_workspace_violation(tmp_path: Path, workspace_guard: None) -> None:
    ...
    result = _result_map(await ReadFileTool().execute(path=str(outside_file)))

    assert result["success"] is False
    assert "WORKSPACE_VIOLATION:" in cast(str, result["error"])
    assert "outside the workspace root" in cast(str, result["error"])
