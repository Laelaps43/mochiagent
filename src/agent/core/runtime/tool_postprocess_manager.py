...
class _AgentPostprocessorBinding:
    __slots__: tuple[str, ...] = ("name", "config", "processor")

    def __init__(
        self, name: str, config: ToolPostprocessorConfig, processor: ToolResultPostProcessorStrategy
    ) -> None:
        self.name: str = name
        self.config: ToolPostprocessorConfig = config
        self.processor: ToolResultPostProcessorStrategy = processor


class ToolPostprocessManager:
    def __init__(self) -> None:
        self._registry: ToolResultPostProcessorRegistry = ToolResultPostProcessorRegistry()
        self._registry.register("default", self._build_default_processor)
        self._default_name: str = "default"
        self._default: ToolResultPostProcessorStrategy = self._registry.create("default")
    ...
    def _build_default_processor(options: ToolPostprocessorConfig) -> ToolResultPostProcessor:
        config = ToolResultPostProcessConfig(
            summary_max_chars=cast(int, options.get("summary_max_chars", 4000)),
            preview_head_chars=cast(int, options.get("preview_head_chars", 1500)),
            preview_tail_chars=cast(int, options.get("preview_tail_chars", 1000)),
        )
